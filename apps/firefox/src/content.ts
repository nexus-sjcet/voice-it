...
// Custom Functions
const extractWord = (span: HTMLSpanElement) => {
    let word = '';
...
const injectPopup = () => {
    ...
    plCSpans.forEach(span => {
        const word = extractWord(span as HTMLSpanElement);
        if (word.match(regexPattern)) {
            ...
            const parent: HTMLDivElement = span.parentNode as HTMLDivElement
            const lineNumber = parent.getAttribute('data-line-number');
            const clickElement = document.querySelector<HTMLElement>(`.react-line-number[data-line-number="${lineNumber}"]`);
